...
const selectHighestLiquidityPrice = (payload: DexResponse): number | null => {
  ...
  for (const pair of pairs) {
    ...
    if (typeof liquidityUsd !== "number") continue;
...
export function createMarketCapService({
  ...
  async function fetchTokenPrice(token: TokenConfig): Promise<number> {
    ...
    try {
      ...
      if (!response.ok) {
        log.error("market-cap.fetch.error", toExternalApiError({ status: response.status, ticker: token.ticker }));
        return 0;
      }
      const json = (await response.json()) as DexResponse;
      const price = selectHighestLiquidityPrice(json);
      if (!price || !Number.isFinite(price)) {
        log.warn("market-cap.price.missing", { ticker: token.ticker });
        return 0;
      }
      if (!token.supply || token.supply <= 0) {
        log.warn("market-cap.supply.missing", { ticker: token.ticker });
        return 0;
      }
      return price * token.supply;
    } catch (error) {
      const message = error instanceof Error ? error.message : "unknown";
      log.error("market-cap.fetch.exception", toExternalApiError({ message, ticker: token.ticker }));
      return 0;
    }
  }
