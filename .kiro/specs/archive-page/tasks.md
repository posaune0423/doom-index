...
## 1. データ構造の基盤整備とメタデータ保存機能の実装

### 1.1 画像生成時のメタデータ構造定義と型システムの構築
...
## 2. R2ストレージからのアーカイブデータ取得機能の実装

### 2.1 R2リストAPIによる画像一覧取得機能の構築
...
## 3. アーカイブAPIエンドポイントの実装

### 3.1 Next.js API Routeの作成とクエリパラメータ処理
...
### 4.2 React Queryによるデータ取得とページネーション管理
...
## 5. ページネーションUIコンポーネントの実装

### 5.1 ページネーションコントロールの作成
...
### 8.2 画像クリック時の詳細表示機能の実装
- 画像クリックイベントを検知する機能を実装する
...
### 9.1 モーダルの基本構造とレイアウトの実装
- 画面中央にモーダルを表示する機能を実装する
- デスクトップとモバイルでレイアウトを切り替える機能を実装する
- 画像表示エリアとメタデータ表示エリアを分割する機能を実装する
- _Requirements: 8.1_

### 9.2 画像の拡大表示とズーム・パン操作の実装
- 高解像度画像を表示する機能を実装する
- デスクトップでマウスホイールによるズーム機能を実装する
- モバイルでピンチズームとスワイプによるパン機能を実装する
- _Requirements: 8.1_
...
### 9.4 モーダルの操作機能の実装
- 閉じるボタンを実装する
- ESCキーでモーダルを閉じる機能を実装する
- オーバーレイクリックでモーダルを閉じる機能を実装する（デスクトップのみ）
- モーダル表示中のスクロールを無効化する機能を実装する
- _Requirements: 8.1_
